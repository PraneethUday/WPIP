...
export default function ClaimsScreen({ navigation }) {
  ...
  const loadClaims = useCallback(async (isRefresh = false) => {
    ...
    setError("");
    try {
      const res = await api.getWorkerClaims(token, user.delivery_id).catch(() => null);
      setClaims(res?.data || []);
  ...
  const filtered       = filter === "all" ? normalized : normalized.filter((c) => c._status === filter);
  const totalReceived  = normalized.filter((c) => c._status === "paid").reduce((a, c) => a + (Number(c.payout_amount) || 0), 0);
                ...
                <View key={key} style={styles.card}>
                  ...
                  <View style={styles.cardBody}>
                    ...
                    <View style={styles.cardHead}>
                      <View style={[styles.cardIconBg, { backgroundColor: sc.bg }]}>
                        <Ionicons name={claim._icon} size={17} color={sc.color} />
                      </View>

                      <View style={styles.cardHeadMid}>
                        <Text style={styles.cardType}>{prettyTrigger(claim.trigger_type)}</Text>
                        <Text style={styles.cardMeta}>
                          {claim.city || user?.city || "Unknown"}{" · "}{formatDate(claim.created_at)}
                        </Text>
                      </View>
                      ...
                      <View style={styles.cardHeadRight}>
                        <Text style={[
                          styles.cardAmount,
                          { color: claim._status === "rejected" ? COLORS.textFaint : COLORS.white },
                        ]}>
                          {claim._status === "rejected" ? "—" : money(Number(claim.payout_amount))}
                        </Text>
                        <View style={[styles.statusChip, { backgroundColor: sc.bg }]}>
                          <View style={[styles.statusChipDot, { backgroundColor: sc.color }]} />
                    ...
                    {claim._status === "paid" && (
                      <View style={[styles.cardFooter, { backgroundColor: COLORS.successContainer }]}>
                        <Ionicons name="flash-outline" size={11} color={COLORS.success} />
...
const createStyles = (COLORS, FONTS) => StyleSheet.create({
  ...
  errorBannerText: { flex: 1, fontSize: SIZES.small, fontFamily: FONTS.medium, color: COLORS.error },
  ...
  heroBadge: {
    ...
    borderRadius: SIZES.radius, paddingHorizontal: 12, paddingVertical: 8,
    alignItems: "center", borderWidth: 1, borderColor: COLORS.borderActive,
  },
  heroBadgeNum: { fontSize: SIZES.h3, fontFamily: FONTS.bold, color: COLORS.primary },
  heroBadgeLabel: { fontSize: 10, fontFamily: FONTS.medium, color: COLORS.primary, textTransform: "uppercase", letterSpacing: 0.5 },
  ...
  heroStatNum: { fontSize: SIZES.body, fontFamily: FONTS.bold, color: COLORS.white },
  heroStatLabel: { fontSize: 11, fontFamily: FONTS.medium, color: COLORS.textFaint },
  ...
  noteText: { flex: 1, fontSize: 12, fontFamily: FONTS.medium, color: COLORS.primary },
  ...
  filterTabText: { fontSize: 11, fontFamily: FONTS.semiBold, color: COLORS.textFaint },
  ...
  filterBadgeText: { fontSize: 9, fontFamily: FONTS.bold, color: COLORS.textFaint },
  ...
  emptyTitle: { fontSize: SIZES.body, fontFamily: FONTS.bold, color: COLORS.white, marginBottom: 6 },
  emptySub: { fontSize: SIZES.small, fontFamily: FONTS.medium, color: COLORS.textFaint, textAlign: "center", lineHeight: 20 },
  ...
  cardHead: {
    ...
    paddingLeft: 11, paddingRight: 13,
  },
  cardIconBg: {
    width: 38, height: 38,
    borderRadius: 11,
  ...
  cardHeadMid: { flex: 1 },
  cardType: { fontSize: SIZES.body, fontFamily: FONTS.bold, color: COLORS.white, marginBottom: 3 },
  cardMeta: { fontSize: 12, fontFamily: FONTS.medium, color: COLORS.textFaint },
  cardHeadRight: { alignItems: "flex-end", gap: 6 },
  cardAmount: { fontSize: 18, fontFamily: FONTS.display, letterSpacing: -0.5 },
  ...
  statusChipText: { fontSize: 10, fontFamily: FONTS.bold, letterSpacing: 0.2 },
  ...
  factVal: { fontSize: SIZES.small, fontFamily: FONTS.semiBold, color: COLORS.white },
