...
export default function PolicyScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (mounted) {
          setNextWeekTier(normalizeTier(savedTier || user.tier));
        }
  ...
  const cityTriggers = useMemo(() => {
    if (!user?.city) return [];
    const cityData = triggerStatus?.[user.city];
    if (!cityData?.triggers_fired?.length) return [];
    return cityData.triggers_fired;
